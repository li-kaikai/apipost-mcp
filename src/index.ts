...
function generateApiTemplate(method: string, url: string, name: string, config: any = {}): any {
  ...
  const generateRequestBody = () => {
    ...
    const arrayElementParams = bodyParams.filter(param =>
      param.key && (param.key.includes('[].') || param.key.includes('[0.]'))
    );
  ...
  return {
    ...
    request: {
      ...
      query: { 
        ...
        body: (() => {
          const bodyParams = config.body || [];
          const hasArrayElements = bodyParams.some((param: any) =>
            param.key && (param.key.includes('[].') || param.key.includes('[0].'))
          );
...
总数: ${totalCount}个, 筛选后: ${filteredCount}个${logInfo}${limitInfo}`);
      ...
      case 'apipost_update':
        ...
        updateTemplate.parent_id = (args.parent_id as string) || originalApi.parent_id || '0';
